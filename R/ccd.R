...
## OLD VERSION - Generate a central composite design
# Oct 2012 added a new argument 'keep': Uses these symbols
#   C -- cube blocks and their center points;  c -- cube blocks without center points
#   S -- star blocks and center points;        s -- star blocks w/o center pts
# This lets us continue to use positive values of n0, wbreps, etc so that alpha
# is computed correctly
# oldccd = function(basis, generators, blocks="Block", n0=4, alpha="orthogonal", 
#                wbreps=1, bbreps=1, randomize=TRUE, inscribed=FALSE, coding, 
#                keep="CS")
# {
#     # save the call and just return that if basis is missing
#     CALL = match.call()
#     if (missing(basis)) {
#         return(CALL)
#     }
...
# 
#     # I'm going to use an extra column 'keep.it' to track whether to keep this portion   
#     cube$keep.it = (length(grep("c", tolower(keep))) > 0)
#     star$keep.it = (length(grep("s", tolower(keep))) > 0) 
#     
#     # center points
#     zero = as.data.frame(matrix(rep(0, k), nrow=1)) 
#     names(zero) = names(cube)[1:k]
#     
#     zero$keep.it = (length(grep("C", keep)) > 0)
...
#     }
#     
#     zero$keep.it = (length(grep("S", keep)) > 0)
#     if (n0[2] > 0) 
...
#     }
#     if (inscribed)
#         cube[, 1:k] = cube[, 1:k] / alpha
#     else
#         star[, 1:k] = star[, 1:k] * alpha
#     
...
#     # assemble design
#     des = rbind(cube, star)
#     des = des[des$keep.it, 1:(ncol(cube)-1)] # keep only the parts we want
#     
#     # Add vars from left-hand side, if any
#     if (inherits(basis, "formula") & (length(basis) > 2)) {
#         yvars = all.vars(basis[[2]])
...
#     
#     attr(des, "call") = CALL
#     rsd.design(des, xvars, blknm, n0, alpha, CALL)
# }

### new version of ccd -- constructs it using cube, star, dupe, foldover, and join
...
# Note: We play tricks and pass a negative value if inscribed is TRUE
    ...
    if (is.character(blocks)) { # need to call cube with missing blockgen
        ...
        if(nfb > 1) for (i in 2:nfb)
            des = join(des, foldover(cp, bid=i, randomize=randomize), blkname=blkname)
        cp = des
    }
    else
    ...
    if(bbreps[1] > 1) for (i in 2:bbreps[1])  {
        des = join(des, dupe(cp, randomize=randomize), blkname=blkname)
    }
    ...
    for (i in 1:bbreps[2])
        des = join(des, dupe(sp, randomize=randomize), blkname=blkname)
...
join = function(design1, design2, ..., blkname = "Block", blocklev) {
