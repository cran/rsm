...
\Abstract{
  This introduction to the \proglang{R} package \pkg{rsm} is a (slightly)
...
described in texts such as \cite{Box87}, \citet[Chapters~1--5]{Khu96}, \citet[Chapter~9]{Wu00}, \cite{Mye02},
...
Accordingly, the \rsm{} package provides for a "coded.data" class of objects, an extension of "data.frame".  The functions "coded.data", "as.coded.data", "decode.data", "code2val", and "val2code" create or decode such objects.  If a "coded.data" object is used in place of an ordinary "data.frame" in the call to other \rsm{} functions such as "rsm" (\Sect{fitting}) or "steepest" (\Sect{steepest}), then appropriate additional output is provided that translates the results to the original units.  The print method for a "coded.data" object displays the coding formulas.

As an example, consider the provided dataset "ChemReact", which comes from Table~7.7 of \cite{Mye02}.
<<>>=
library("rsm")
ChemReact
@
In this experiment, the data in block "B1" were collected first and analyzed, after which block "B2" was added and a new analysis was done.  
Note that block "B1" uses factor settings of $\text{Time}=85\pm5$ and $\text{Temp}=175\pm5$, with three center points.  Thus, the coded variables are $x_1 = (\text{Time}-85)/5$ and $x_1 = (\text{Temp}-175)/5$.  To create a coded dataset with the appropriate codings, provide this information via formulas:
<<>>=
...
<<>>=
code2val(data.frame(x1 = c(0.25, 0.5), x2 = c(-1.5, -0.5)),
  attr(CR, "codings"))
@
...
The choice of which design is best depends on the tradeoff between economy and ability to assess the fitted surface.  Design~1 is the only one of these that is included in Table~7.6 of \cite{Mye02}.  It is good to be able to look at a broader range of choices.

Once we decide the design, the "ccd" function is used to generate it. We first illustrate the generation of Design~1 above.  This design requires a $2^{5-1}$ fraction for the cube block.  Typically, this is done by confounding the five-way interaction with the mean; or equivalently, create a four-factor design, and generate the levels of the fifth as the four-way interaction of the others.  That is the approach implemented by "ccd".
...
@
Each block is randomized separately, but the order of the blocks is not randomized---we may not want to run them in sequence.  
With this design, just one of the cube blocks is sufficient to estimate a first-order response surface.  What
is less obvious, but true, is that we can run any two of the cube blocks plus the axis block and obtain
sufficient data to estimate a second-order response surface.  To check this (which is a good idea in advance
of collecting data), create a fake vector of response values, and use the "rsm" function (see next section) to
fit a second-order surface to the subset "(Blk == 1 | Blk == 2 | Blk == 5)".  It would produce an error message if the design were inadequate; but it doesn't.  In summary, this design provides for a lot of flexibility in how to proceed.
...
To go further, we need more data.  Thus, let us now presume that the data in the second block have been collected, so that there is data from a full central-composite design.  We are now in the position of fitting a full second-order model.  This can be done by adding "PQ(x1, x2)" to the above model with interaction, but the easier way is to use "SO", which is shorthand for a model with "FO", "TWI", and "PQ" terms.  Also, we now need to account for the block effect since the data are collected in separate experiments:
<<>>=
...
the coordinate axes.  Thus, the upper-left plot depicts a slice of the contour surface in $x_1$ and $x_2$,
where $x_3 = -0.839$ and $x_4 = -0.116$, the stationary values.  The maximum for this slice is at coordinates $x_1 =
0.861, x_2 = -0.331$.
...
<<>>=
steepest(CR.rsm1, dist = c(0, 0.5, 1))
@
...
I wish to thank the students in my Industrial Experimentation course (fall semester 2008) who were helpful in defining the requirements needed for \rsm{}, as well as in debugging it.
